...
When writing code to make calls to LLMs, use the OpenAI SDK directly against the `gpt-4o-mini` model (cheapest mini-class model in the OpenAI lineup). Use Structured Outputs (the SDK's `response_format` with a Pydantic / Zod schema, or the `responses.parse` helper) so the result is a typed object you can use to populate fields in the legal document.
...
- Uses the OpenAI SDK with the `gpt-4o-mini` model
